...
export const createCookProfile = asyncHandler(async (req, res) => {
  const existing = await CookProfile.findOne({
    user: req.user._id,
  });

  if (existing) {
    throw ApiError.conflict(
      'Cook profile already exists'
    );
  }
  ...
  return ApiResponse.created(
    res,
    profile,
    'Cook profile created'
  );
});

// GET /api/cooks/profile/me
export const getMyCookProfile =
  asyncHandler(async (req, res) => {
    let profile =
      await CookProfile.findOne({
        user: req.user._id,
      }).populate(
        'user',
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      ...
      throw ApiError.forbidden(
        'Only approved cooks can toggle availability'
      );
    }
...
export const uploadKitchenImages =
  asyncHandler(async (req, res) => {
    if (
      !req.files ||
      req.files.length === 0
    ...
    if (
      ...
      throw ApiError.badRequest(
        'Maximum 8 kitchen images allowed'
      );
    }
...
export const deleteKitchenImage =
  asyncHandler(async (req, res) => {
    const profile =
      await CookProfile.findOne({
        user: req.user._id,
...
export const uploadVerificationDocs =
  asyncHandler(async (req, res) => {
    if (!req.file) {
      throw ApiError.badRequest(
...
export const getNearbyKitchens =
  asyncHandler(async (req, res) => {
    const {
      latitude,
      longitude,
